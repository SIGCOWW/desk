...
具体的にはこんな感じです。
@<tt>{image}の代わりに@<tt>{limage}および@<tt>{rimage}という命令を利用することで、左右に余白を付けることもできます。
特に、画像2枚でそれぞれ@<tt>{limage}と@<tt>{rimage}を使うと、中央揃えのようなことが実現可能です。
//emlist{
 //subfig[キャプション]{
...
=== 吹き出し
+メッセージやSlackで見かけるような吹き出しとして、@<tt>{//chat}ブロックを用意しました。
第1引数で画像を指定し、ブロックに吹き出したい内容を記述すると、それらしい感じで出力します。
画像については@<tt>{//image}のように適切な場所から検索してくれます。
実際の例は次のとおりです。
//chat[チノ_ジト目]{
うるさいですね…
//}
画像と吹き出しの位置関係が逆になった、@<tt>{//rchat}ブロックもあります。
//rchat[チノの父]{
このゲームは、君がココア君になって、友人達と仲良く日々を過ごすゲームだ。
このゲームは、@<bou>{君が}ココア君になって、友人達と仲良く日々を過ごすゲームだ。

このゲームは、君がココア君になって、友人達と仲良く日々を過ごすゲームだ。
//}

ここでの画像指定についてですが、日本語で記述することもあるでしょう。
と言うのも、EPUB版では@<b>{画像ファイル名の「_」以前をそのまま出力する}ためです。
例えば「チノ」と指定した場合、LaTeX版では「チノ.png」などの画像を出力し、EPUB版では「チノ」という文字列が出力されます。
「チノ_ジト目」と指定した場合、LaTeXでは画像「チノ_ジト目.png」が、EPUB版では文字列「チノ」が出力されます。
