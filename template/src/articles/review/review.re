...
なおLaTeXにおいて、脚注のような幅が狭いところに文字を出力した場合、改行がうまく制御できず見栄えが悪くなることがあります。
...
さらに、これもdesk特有ですが、@<tt>{//subfig}および@<tt>{w}版の@<tt>{//subfigw}命令を追加しました。
...
そこに表示される内容を設定するため、@<tt>|@<author>{}|と@<tt>{//profile}という2つの命令を実装しました。
...
現時点では「@<tt>{著者名;tw:アカウント名}」とすることで、twitterのアカウントを設定できます。
...
=== アイキャッチ
ページ数調整の目的で、@<tt>{//eyecatch}命令を用意しました。
第1引数に@<tt>{high}・@<tt>{mid}・@<tt>{low}をとり、ブロックの内容をそれらしい感じに出力します。
